...
Api = '6531817805:AAGsktPx5fcuXkba6Hz85ZwQyrzE1ZeHBVQ'
bot = Bot(token= Api)
dp = Dispatcher(bot)

@dp.message_handler(commands=['start'])
async def start(message: types.Message):
    await message.answer('Нормальная у нас нейронка')


def generate_image(prompt_text):
    prompt = {
        "modelUri": "art://b1gak84qsqs59igorb4j/yandex-art/latest",
        "generationOptions": {
    ...
    headers = {
            ...
            "Authorization": "Api-Key AQVNzRKbTe_DsjDz7p3TsGcdRPbqu2CLPd_CAyxB"
        }
    ...
    while True:
        ...
        if 'response' in operation_result:
            ...
            print("Подожди, пока сделаю изображение")
            time.sleep(5)
